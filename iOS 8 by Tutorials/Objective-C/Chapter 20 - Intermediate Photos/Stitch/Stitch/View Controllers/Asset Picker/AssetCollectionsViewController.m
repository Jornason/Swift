...
@interface AssetCollectionsViewController () <PHPhotoLibraryChangeObserver>
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  if (indexPath.section == 0) {
    ...
    destination.assetsFetchResults = [PHAsset fetchAssetsInAssetCollection:album options:nil];
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    BOOL updatedFetchResults = NO;
    
    PHFetchResultChangeDetails *changeDetails = [changeInstance changeDetailsForFetchResult:self.userFavorites];
    if (changeDetails) {
      self.userFavorites = [changeDetails fetchResultAfterChanges];
      updatedFetchResults = YES;
    }
    
    changeDetails = [changeInstance changeDetailsForFetchResult:self.userAlbums];
    if (changeDetails) {
